...
一次性读写文件内容应优先选用 string.load string.save 而不是使�?io.file 创建文件对象�?
参考： [io.file 文件对象](../io/file.html) [检测文件路径](../io/path.html#exist)

## 读文�?
`string.load(path,resType="RES",dllHandle = null )`

string.load 有三个参数，第一个参�?@path 为文件路径�?
...
函数的第三个参数 @dllHandle 指定查找资源文件的执行文件内存模块的句柄(pointer 指针类型)，不指定时默认为当前 EXE 模块�?使用 [raw.loadDll](../raw/api.html#loadDll) 可以加载dll 对象，dll对象提供 gethandle() 函数返回 dll 模块句柄�?
## 写文�?
