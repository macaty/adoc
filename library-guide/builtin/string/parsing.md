...
�?[string.match](matching.html#match) 等模式匹配函数不同，string.each 会将文本按行拆分，然后返回一个迭代器，用于在 [for in](../../../language-reference/statements/iterator.html) 语句中循环且逐行匹配字符串并且循环返回结果�?
- 参数 str 指定要处理的字符串�?- 参数 pattern 指定模式匹配串，查找模式串中可用圆括号创建捕获分组自定义迭代器返回值个数，如果不指定捕获组则迭代器直接返回找到的字符串�?
示例�?
```aardio aardio
import console;
...
说明�?
string.lines 用于创建一个迭代器，用�?[for in](../../../language-reference/statements/iterator.html) 语句中循环按行拆分字符串并返回拆分结果�?
- 参数 str 指定要解析的字符串�?
...
//转换为数�?var array = table.array( , string.lines(str,,",") )

```

如果迭代器传�?table.array 的第 2 个参数，则取迭代器的首个循环返回值作为数组元素�?
## 三、解析字符串属性表 [\#](\#talbe)
...
格式说明�?
- 每行为一个单位，属性值不能跨行，引号等仅作为字面值解析�?- 名值对之间�?`:` �?`=` 分隔，忽略分隔等前后的空白字符�?- 某些程序里允许在行首�?`#` 标明一个需要被忽略的注释�?
aardio 提供�?string.table 用于快速解析这样的文本并返回一个表对象�?
示例�?
```aardio aardio
import console;
...
除了第一个参数，其他参数都是可选的�?
参数 nameValueSeparator 用于指定键值分隔符，已默认指定�?`\s*[\:=]\s*` �?
参数 lineDelimiter 用于指定识别行分隔符的模式串，已默认指定�?`<\r*\n>|\r` �?
参数 commentChar 指定行首注释符，这个参数仅支持字节码。默认未指定。如果指定为 `'#'#` 表示�?`#` 置于行首时则忽略该行�?
