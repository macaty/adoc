...
attr 是一个用字节码表示的数值，中文多字词的默认值为 `'@'#`，单字的值为 0，英文字�?attr 的值为 `'e'#`。我们可以在词库中的多字词后面加一个字格再加一个标点符号可以预定义一个词�?attr �?
可以使用 mmseg.loadChars 函数加载字库，使�?mmseg.loadWords 函数加载词库。mmseg 已经加载了默认的字词库，�?mmseg.loadChars �?mmseg.loadWord 会自动忽略不存在的文件，所以可以根据需要删除或替换字词库。要特别注意字词库要自行除重，尽量不要加入重复的词，这样会导致分配多余内存�?
...
## 使用 string.sentences 实现文本分句 [\#](\#sentences)

分句指的是文本拆分为一个个语意相对独立的语句�?
aardio 提供�?string.sentences 可用于中英文的文本分句，示例�?
```aardio aardio
import console;
