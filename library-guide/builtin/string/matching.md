...
2. 说明�?
   使用第二个参�?@pattern 指定的模式串查找第一个参�?@str 指定的字符串并返回起始位�?i),结束位置(j)�?
   此函数支�?[模式匹配语法](patterns.html) 。如果需要禁用模式语法，请在模式串开始添�?`@` 字符，或者使�?[不使用模式匹配语法的查找字符串函数](search.html)�?
   可选用第三个参�?@pos 指定搜索开始的位置（以字节计数），省略则使用默认�?1 �?@pos 如果为负数，则从右侧倒数计数(-1表示字符串最后一个字�?�?
   可选使用第四个参数 @chars 指定返回值是否以字符计数，设�?true 则返回值以字符计数，否则返回值以字节计数。注意，@pos 始终以字节计数�?
   函数返回匹配字符串的起始位置 i ，结束位�?j 。如果查找失�? i,j 都返�?null 值�?
3. 示例
   ...
   if( i ) //如果找到i为大�?的数�?   console.log("�?'abc456d' 中找�?c\d+" ,"�? + i + "�? + j);

   if(not string.find("abcd" ,"不可�?) )
       console.log("没有�?'abcd' 中找�?'不可�? ");
   ...
   每个捕获组都会增加一个返回字符串，返回值的前后顺序也就是模式串中捕获组的左圆括号出现的前后顺序。参考： [模式语法 \- 捕获组](patterns.html#capturing-groups)
...
2. 说明�?
   第一个参�?@str 指定目标字符串，每二个参数指�?@pattern 查找模式串。第三个参数 @pos 可选指定开始位置（以字节计数）�?
   这个函数�?string.find 很象。但是返回匹配的开始位置与结束位置。而是返回找到的字符串�?
...
3. 示例�?
   ...
   ```aardio aardio
   //\d表示数字 \d+表示最长匹配多个数�?   var v = string.match("abcd1234", "\d+");
...
2. 说明�?
   ...
   迭代器每执行一次就返回该次匹配到的结果，返回匹配结果的规则�?string.match 相同�?
   ...
   参考： [模式语法 \- 捕获组](patterns.html#capturing-groups)
...
3. 示例�?
   ...
   //for后面的变量对应查找模式串中的括号分组(有几对括号就有几个返回�?
   ...
   参考： [泛型 for 与迭代器](../../../language-reference/statements/iterator.html)
...
2. 说明�?
   第一个参�?@str 指定目标字符串�?
   第二个参�?@pattern 指定搜索模式串，支持模式匹配语法�?
   第三个参�?@repl 指定替换对象，可以是字符串、替换表、替换函数�?
   第四个可选参数用于限定替换的次数，默认替换所有的找到的字符串�?
   如果需要禁用模式语法，请在模式串开始添�?`@` 字符�?
   函数返回�?str 为替换结果，返回�?count 为替换次数�?
3. 示例�?
...
替换字符串中可以�?`\1,\2,\3.....\9` 等表示模式匹配后的捕获组引用�?
请参考： [模式语法 \- 捕获组引用](patterns.html#backreferences)
...
//在模式匹配中 .圆点标记匹配任意字符�?str = string.replace("abcd","(.)","\1k");
...
注意在查找串中可以引用的捕获组为 `\1` �?`\9` �?而在替换串中除了可以引用 `\1` �?`\9` 的捕获组，还可以�?`\0` 完整的匹配字符串�?
下面�?aardio 代码互换相邻的字�?
...
第三个参�?@repl 也可以指定一个替换函数。替换函数的回调参数为所有捕获组对应的字符串，如果没有捕获组则回调参数为每次匹配到的字符串。替换函数如果返回字符串则执行替换，如果返回 null 或者不返回值则不执行替换�?
使用替换函数的示例：

```aardio aardio
import console;

//如果两个捕获都是数字则相加，否则将位置前后掉�?function captureProc(a,b) {
    console.log(owner,a,b); //owner 显示原始字符�?    if(tonumber(a) and tonumber(b) )
        return a + b;//如果两个捕获都是数字则相�?    else
        return b ++ a;//否则将位置前后掉�?}
...
## 五、在不匹配模式的部分进行替换 [\#](\#replaceUnmatched)
...
但如果我们要在不匹配一个或多个模式的部分替换要，那就需要用�?string.replaceUnmatched 函数了�?
函数原型�?
`str = string.replaceUnmatched(str, pattern, repl , keep1 [ , keep2, ... keepN ] )`

string.replaceUnmatched �?str 参数指定要进行替换的源字符串，�?pattern 指定替换模式串，repl 参数指定替换对象，这些参数的用法�?string.replace 函数完全一样�?repl 参数同样可以指定替换字符串、替换表或替换回调函数�?
