...
aardio,PHP,C#,Java 等不同编程语言�?AES 加解密结果要保持一致要注意以下一些要点：

1. 工作模式 CBC ，填充模�?PKCS7，不同语言要保持一致�?   要注�?PKCS5 �?PKCS7 的填充规则是相同的，区别是PKCS5填充1�?字节,PKCS7填充1�?55字节，而AES实际使用的数据区分组�?16 字节�?28位），所以即使填充模式指�?PKCS5 - 实际使用的也�?PKCS7。下面的 JAVA代码里只能�?PKCS5 ，�?C#代码里只能�?PKCS7, 这都是兼容的没有问题�?2. 在下面的示例中，加密向量统一设为与密钥相同�?3. 不同编程语言使用的文本编码要一致，同一个字符串，使用UTF8或GBK编码在内存中存储的实际数据可能是不一样的。在aardio中默认编码为UTF-8,使用 string.fromto进行转换为其他编码�?4. 如果加密后返回的密文用了BASE64�?6进制编码，那么在解密时同样也先做对应的逆向解码�?
...
## 要保�?AES 加解密结果与上面�?aardio 代码一致， C\# 语言 AES 加解密代码如下：
...
## 要保�?AES 加解密结果与上面�?aardio 代码一致， PHP 语言 AES 加解密代码如下：
...
## 要保�?AES 加解密结果与上面�?aardio 代码一致， Java 语言 AES 加解密代码如下：
