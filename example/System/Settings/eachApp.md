...
/*DSG{{*/
var winform = win.form(text="UWP 搴旂敤 - 鏌ョ湅宸ュ叿";right=1150;bottom=666;bgcolor=16777215)
winform.add(
...
openMicrosoftStore={cls="plus";text="鎵撳紑搴旂敤鍟嗗簵";left=974;top=625;right=1110;bottom=657;color=8388608;db=1;dr=1;notify=1;textPadding={left=5};z=4};
    ...
    select(code) {
        case 0xFFFFFFFB/*_NM_RCLICK*/  {
            ...
            if(1===#winform.listview.selected){
                ...
                winform.popmenu.add("杩愯",function(id){
                    import com.shell;
...
winform.plusSearch.setCueBannerText("杈撳叆搴旂敤鍚嶏紙鏀寔妯″紡鍖归厤锛?);
winform.listview.setExtended(0x10000/*_LVS_EX_DOUBLEBUFFER*/);
winform.listview.insertColumn("搴旂敤鍚?,300)
...
if(!_WIN10_LATER){
    winform.msgWarn("鏈▼搴忎粎鏀寔 Windows 10");
}
...
winform.openMicrosoftStore.oncommand = function(id,event){
    ...
    //鎵撳紑搴旂敤鍟嗗簵锛屼笖淇 ms-windows-store:// 鍗忚
    com.shell.activateApp("Microsoft.WindowsStore_8wekyb3d8bbwe!App");

    /*
    鏈夋椂鍊欑郴缁熷嚭鐜伴棶棰?ms-windows-store 鍗忚鍙兘鐢ㄤ笉浜嗭紝涓嬮潰鐨勬柟娉曞氨鎵撲笉寮�銆?    浣?com.shell.activateApp("Microsoft.WindowsStore_8wekyb3d8bbwe!App") 鍙互鎵撳紑銆?    鑰屼笖鎵撳紑涓�娆★紝ms-windows-store 鍗忚灏辨甯镐簡銆?    */
    //process.execute("ms-windows-store://home")
}

winform.editCode.setCueBannerText("鍗曞嚮鍒楁爣棰樺彲鎺掑簭锛屽弻鍑诲垪琛ㄩ」鍙鍒讹紝鍙抽敭鐐瑰垪琛ㄩ」鍙脊鍑鸿彍鍗曪紝鍥炶溅鎼滅储鎴栧埛鏂?);
win.loopMessage();

```

[Markdown 鏍煎紡](javascript:if(confirm('https://www.aardio.com/zh-cn/doc/example/System/Settings/eachApp.md  \n\n该文件无法用 Teleport Ultra 下载, 因为 它不在项目文件类型规范内。  \n\n你想在服务器上打开它?'))window.location='https://www.aardio.com/zh-cn/doc/example/System/Settings/eachApp.md')
