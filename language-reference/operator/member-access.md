...
# 成员操作�?
参�?
[表对象](../datatype/table/_.html) [元表](../../library-guide/builtin/table/_.html) [重载操作符](overloading.html)
...
当使用形�?tab.member() 的格式调�?[\_get 元方法](../datatype/table/meta.html#_get) �?ownerCall 参数�?true。如果只是写 tab.member 获取值，�?ownerCall 参数�?false。如果是�?tab\["member"\] 的格式则 \_get 元方法的 ownerCall �?null。\[\]var a = tab\["member"\]下标操作符，又称“索引操作符”�?
使用形如 owner\[表达式\] �?owner\["键名"\] 的下标操作符用于赋值语句在触发 [\_set 元方法](../datatype/table/meta.html#_set) �?ownerIndex 参数�?true。用于获取键值并触发 \_get 元方法时 ownerCall 参数�?null ，可此用此特性在语法上区�?owner.键名 �?owner\["键名"\] 的不同作用。`[[]]`var a = tab\[\["member"\]\]直接下标操作符（raw subscript operator�?
