...
在函数体内部可以选用 `return` 语句返回一个或多个值，如果不返回任何值或者不调用 `return` 语句函数会默认返�?null 值�?
`return` 语句之后直到函数结束的代码将视为注释注略不执行，

## 使用函数返回�?
�?aardio 中函数是纯函数（Pure Function�? 输入输出数据流都是显式（Explicit）的�?
函数从函数外部接受的所有输入信息都通过参数传递到该函数内部；函数输出到函数外部的所有信息都通过返回值传递到该函数外部，即函数的数据只有一个入�? 参数 )，一个出�? 返回�?)。函数可以返回多个值�?
示例如下�?
```aardio aardio
import console;

//定义一个函�?var func = function(){
    //函数有多个返回�?    return 1,2,3;
}
...
//把所有的返回值放到表构造器中生成一�?table�?var tab = { func() }; //tab 的值为 {1,2,3}

//这里 func 仅返回一个�?console.log( func() , "123" );

//只有在参数列表的尾部才能接收函数的多个返回�?console.log( "123" , func() );
...
通常成功时返回值的逻辑值为真，失败时通常会返�?个值，分别�?null,错误信息 �?
有时候也会返�?false,错误信息�?
...
3. 调用示例�?
   ...
   //从第二个参数开始返�? re==2,re2==3
   var re,re2 = rget(2,func() );
   console.log( re,re2 )

   //从倒数第一个参数开始返�? re==3,re2==null
...
�?table.push() 函数接受不定个数的参数，它会将所有参数添加到数组的尾部�?`table.push( tab,tonumber("123") )` 在执行时就变成了 `table.push( tab,123,3 )` �?
我们可以向下面这样加一对圆括号�?
`table.push( tab,( tonumber("123") ) );`
