...
each = function(){
    ...
    //下面返回一个迭代器 - 实际上就是一个函数�?    return function(){
        i++;
        if( i < 10 ) return i; //迭代器函数会被循环调�?直到他返回的是空�?    }
}
...
从上面我的代码我们了解到�?
> 1、迭代器实际上就是一个函数对�?这个函数会在for语句中被多次的循环调�?>
...
通过学习 aardio 基础语法，我们知道在调用一个对象的成员函数�?- 函数默认会提�?owner 参数指向他所在的集合�?
所以如果我们把上面�?each( array ) = function(){}  函数修改�?arr.each = function(){}  以后，就可以在迭代器内部使用 owner 对象来访问集合对象了�?
我们想当然地把上一节的代码修改如下�?
```aardio aardio
import console;
...
可是运行后，代码报错了，说是 onwer 参数�?null，出错行为上面红字标出的那行�?
我们知道每个函数都有自己�?owner 参数，arr.each 函数内部�?owner 参数无疑就是指向 arr 对象�?
但在 arr.each 内部创建的新函数�?也就是那个迭代器 ）并不是任何集合对象的成员，他的 owner 对象是空的�?
> 关于 owner 参数的详细说明，请参考这里： [owner 参数](../../language-reference/function/owner.html)

这该如何是好呢？�?
幸运的是 \- aardio 中提供了一种机制，for 语句可以在接受迭代器生成器的多个返回值，你可以用第一个返回值返回迭代器，你可以用第二个返回值返回迭代器�?owner 对象，正确的代码如下�?
```aardio aardio
import console;
...
arr.each = function(){
    ...
    return function(){
        i++;
        if( owner[ i ] ) return owner[ i ];
    },owner //使用第二个返回值显式指定迭代器�?owner 参数
}
...
而依�?for 语句来传递迭代器集合对象、以及状态值的叫“无状态的迭代器”，也就是说这种迭代器依�?for 语句来保持状态�?
我们前面说过了，for 语句在循环开始首先调用迭代器生成器，迭代器生成器可以返回以下三个返回值�?
> 迭代�?集合对象,控制变量 = 迭代器生成器( 可选的生成器参�?)

当然集合对象,控制变量这两个返回值都是可选的�?
如果迭代器生成器返回了集合对象，那么 for 语句会在每次调用迭代器时把他指定为迭代器�?owner 参数�?
...
var each = function(集合对象){
    ...
    return function(控制变量){
        ...
        控制变量++;
        if( owner[控制变量] ) return 控制变量/*新的�?/,owner[控制变量];
...
�?aardio 中有一个规则，所有对象提供的迭代器生成器的函数名都以"each"开始，例如�?
```aardio aardio
import console;
...
所以，请善�?aardio 编辑器提供的智能提示找到您需要的迭代器生成器�?
